...
export const api={
 ...
 listLeads:p=>list('leads',p),getLead:id=>request(`/leads/${id}`),getLeadActivity:id=>request(`/leads/${id}/activity`),createLead:i=>create('leads',i),updateLead:(id,i)=>patch('leads',id,i),addLeadNote:(id,i)=>postNested('leads',id,'notes',i),convertLead:(id,i={})=>request(`/leads/${id}/convert`,{method:'POST',body:JSON.stringify(i)}),
 listClients:p=>list('clients',p),getClient:id=>request(`/clients/${id}`),createClient:i=>create('clients',i),updateClient:(id,i)=>patch('clients',id,i),
 ...
 listAttachments:(entityType,entityId)=>list('attachments',{entityType,entityId}),createAttachment:i=>create('attachments',i),
 ...
 getPlannedVsActualHours:p=>request(`/reports/planned-vs-actual-hours${(()=>{const clean=Object.fromEntries(Object.entries(p||{}).filter(([,v])=>v!==undefined&&v!==''));const q=new URLSearchParams(clean).toString();return q?`?${q}`:''})()`),
