...
export default function LeadForm({ employees=[], clients=[], onSubmit, onCancel, submitting }) {
  const [form,setForm]=React.useState({name:'',company:'',phone:'',email:'',source:'',requirement:'',estimatedValue:'',status:'NEW',assignedToId:'',clientId:'',followUpAt:''});
  ...
  return <form className="entity-form" onSubmit={submit}>
    ...
    <div className="form-grid form-grid-2">
      ...
      <label className="form-field"><span>Status</span><select value={form.status} onChange={e=>set('status',e.target.value)}>{['NEW','CONTACTED','QUALIFIED','PROPOSAL','NEGOTIATION','WON','LOST'].map(x=><option key={x}>{x}</option>)}</select></label>
    ...
    <div className="modal-actions"><button type="button" className="secondary" onClick={onCancel}>Cancel</button><button className="primary" disabled={submitting}>{submitting?'Saving…':'Create lead'}</button></div>
  </form>;
}
