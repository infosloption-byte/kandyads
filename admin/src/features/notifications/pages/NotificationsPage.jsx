...
export default function NotificationsPage(){
  ...
  const columns=[{key:'title',label:'Notification',render:n=><div><strong>{n.title}</strong><div className="muted">{n.message}</div></div>},{key:'type',label:'Type',render:n=><span className="table-status">{n.type}</span>},{key:'createdAt',label:'Received',render:n=>new Date(n.createdAt).toLocaleString()},{key:'readAt',label:'State',render:n=>n.readAt?'Read':<strong>Unread</strong>},{key:'action',label:'Actions',render:n=>!n.readAt&&<div className="table-actions"><button className="secondary" onClick={()=>mark(n.id)}><CheckCheck size={14}/> Mark read</button></div>}];
  return <><div className="page-head"><div><p className="eyebrow">COMMAND CENTER / NOTIFICATIONS</p><h1>Notifications <span className="count-badge">{unread} unread</span></h1><p>Operational alerts for assignments, task deadlines and inventory thresholds.</p></div><div className="page-head-actions"><button className="secondary" onClick={generate}><RefreshCw size={15}/> Refresh alerts</button>{unread>0&&<button className="primary" onClick={readAll}><CheckCheck size={15}/> Mark all read</button>}</div></div><section className="panel table-panel"><ResourceTable columns={columns} rows={rows} loading={loading} error={error}/></section></>;
}
