...
export default function EnquiryForm({ onSubmit, onCancel, submitting=false }) {
  ...
  async function submit(event){
    ...
    if(!form.number.trim()||!form.clientId||!form.requirement.trim()){setError('Enquiry number, client and requirement are required.');return;}
    try{await onSubmit({...form,clientId:Number(form.clientId),targetDate:form.targetDate||undefined});}catch(e){setError(e.message||'Unable to save enquiry.');}
  }
  return <form className="entity-form" onSubmit={submit}>
    <div className="form-grid two">
      <label>Enquiry number<input value={form.number} onChange={set('number')} placeholder="ENQ-0001" required/></label>
      ...
      <label>Priority<select value={form.priority} onChange={set('priority')}><option>NORMAL</option><option>LOW</option><option>HIGH</option><option>URGENT</option></select></label>
    ...
    <div className="form-actions"><button type="button" className="secondary" onClick={onCancel}>Cancel</button><button className="primary" disabled={submitting||loadingClients}>{submitting?'Saving…':<><Save size={16}/> Save Enquiry</>}</button></div>
  </form>;
}
