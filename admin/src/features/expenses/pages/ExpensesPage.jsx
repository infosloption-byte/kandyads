...
export default function ExpensesPage(){
 ...
 return <><div className="page-head"><div><p className="eyebrow">COST CONTROL</p><h1>Expenses</h1><p>Record project, job and general operating expenses with approval status.</p></div><button className="primary" onClick={()=>setOpen(true)}><Plus size={16}/> Add expense</button></div><div className="toolbar"><div className="search-box"><Search size={16}/><input value={query} onChange={e=>setQuery(e.target.value)} placeholder="Search expense number or notes"/></div></div><ResourceTable columns={columns} rows={rows} loading={loading} error={error}/><Modal open={open} title="Create expense" description="Record a direct project/job cost or general expense." onClose={()=>setOpen(false)} width="760px"><ExpenseForm categories={categories} projects={projects} jobs={jobs} employees={employees} onSubmit={submit} submitting={submitting}/></Modal></>;
}
