...
export default function OutsourcingPage(){
 ...
 return <><div className="page-head"><div><p className="eyebrow">THIRD-PARTY PRODUCTION</p><h1>Outsourcing</h1><p>Track vendor production orders and agreed external costs.</p></div><div className="page-head-actions"><button className="primary" onClick={()=>setOpen(true)}><Plus size={16}/> New order</button></div></div><section className="panel table-panel"><div className="toolbar"><div className="search-box"><Search size={16}/><input value={query} onChange={e=>setQuery(e.target.value)} placeholder="Search order or scope"/></div></div><ResourceTable columns={columns} rows={rows} loading={loading} error={error}/></section><Modal open={open} title="New outsource order" description="Assign a production scope to a vendor." onClose={()=>setOpen(false)} width="760px"><OutsourceOrderForm jobs={jobs} vendors={vendors} onSubmit={submit} submitting={submitting}/></Modal></>;
}
