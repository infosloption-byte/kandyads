...
export default function TaskDetailPage(){
  ...
  return <div className="task-detail">
    ...
    <div className="task-hero"><div><p className="eyebrow">TASK</p><h1>{t.title}</h1><p>{t.job?.number||'—'} · {t.job?.project?.name||'—'}</p></div><select value={t.status} disabled={saving} onChange={e=>changeStatus(e.target.value)} aria-label="Task status">{statuses.map(s=><option key={s}>{s}</option>)}</select></div>
