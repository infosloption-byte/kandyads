...
const initial={number:'',name:'',clientId:'',quoteId:'',ownerId:'',startDate:'',dueDate:'',value:''};

export default function ProjectForm({onSubmit,onCancel,submitting=false}){
  const [form,setForm]=React.useState(initial),[clients,setClients]=React.useState([]),[quotes,setQuotes]=React.useState([]),[error,setError]=React.useState(''),[loading,setLoading]=React.useState(true);
  React.useEffect(()=>{Promise.all([api.listClients({pageSize:100}),api.listQuotes({pageSize:100,status:'ACCEPTED'})]).then(([c,q])=>{setClients(c.data||[]);setQuotes(q.data||[])}).catch(e=>setError(e.message||'Unable to load project options')).finally(()=>setLoading(false));},[]);
  ...
  </div>{error&&<div className="form-error">{error}</div>}<div className="form-actions"><button type="button" className="secondary" onClick={onCancel}>Cancel</button><button className="primary" disabled={submitting||loading}>{submitting?'Saving…':<><Save size={16}/> Save Project</>}</button></div></form>;
}
