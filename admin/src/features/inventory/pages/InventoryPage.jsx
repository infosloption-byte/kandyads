...
export default function InventoryPage(){
  ...
  return <>
    ...
    <div className="stat-grid"><div className="stat"><span>Materials</span><strong>{summary.length||materials.length}</strong></div><div className="stat"><span>On hand</span><strong>{qty(onHand)}</strong></div><div className="stat"><span>Reserved</span><strong>{qty(reserved)}</strong></div><div className="stat"><span>Available</span><strong>{qty(available)}</strong></div><div className="stat"><span>Reorder alerts</span><strong>{alerts.length}</strong></div></div>
    <section className="panel table-panel"><div className="panel-head"><div><span>STOCK CONTROL</span><h2>Stock overview</h2></div><span className="table-count">{summary.length} materials</span></div><ResourceTable columns={summaryColumns} rows={summary} loading={loading} /></section>
    <section className="panel table-panel"><div className="panel-head"><div><span>ALERTS</span><h2><AlertTriangle size={17}/> Reorder alerts</h2></div><span className="table-count">{alerts.length} alerts</span></div><ResourceTable columns={alertColumns} rows={alerts} loading={loading} /></section>
    <section className="panel table-panel"><div className="panel-head"><div><span>MOVEMENT HISTORY</span><h2>Stock ledger</h2><p>Filter the complete movement history by material, warehouse and movement type.</p></div></div>
      <div className="toolbar">
        <label className="toolbar-select"><span>Material</span><select value={ledgerFilters.materialId} onChange={e=>updateFilter('materialId',e.target.value)}><option value="">All materials</option>{materials.map(m=><option key={m.id} value={m.id}>{m.sku} · {m.name}</option>)}</select></label>
        <label className="toolbar-select"><span>Warehouse</span><select value={ledgerFilters.warehouseId} onChange={e=>updateFilter('warehouseId',e.target.value)}><option value="">All warehouses</option>{warehouses.map(w=><option key={w.id} value={w.id}>{w.name}</option>)}</select></label>
        <label className="toolbar-select"><span>Movement</span><select value={ledgerFilters.type} onChange={e=>updateFilter('type',e.target.value)}>{movementTypes.map(v=><option key={v} value={v}>{v?v.replaceAll('_',' '):'All movements'}</option>)}</select></label>
        <div className="button-group"><button className="secondary" onClick={resetFilters}>Clear</button><button className="primary" onClick={()=>loadLedger(1)}>Apply</button></div>
      </div>
      <ResourceTable columns={ledgerColumns} rows={rows} loading={ledgerLoading}/>
      <div className="page-actions"><span className="table-count">Page {ledgerMeta.page} of {ledgerMeta.totalPages} · {ledgerMeta.total} movements</span><div className="button-group"><button className="secondary" disabled={ledgerLoading||ledgerMeta.page<=1} onClick={()=>loadLedger(ledgerMeta.page-1)}>Previous</button><button className="secondary" disabled={ledgerLoading||ledgerMeta.page>=ledgerMeta.totalPages} onClick={()=>loadLedger(ledgerMeta.page+1)}>Next</button></div></div>
    </section>
