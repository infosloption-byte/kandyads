...
export default function VendorsPage(){
  ...
  return <><div className="page-head"><div><p className="eyebrow">SUPPLIERS / PARTNERS</p><h1>Vendors</h1><p>Maintain third-party suppliers and production partners.</p></div><div className="page-head-actions"><button className="primary" onClick={()=>setOpen(true)}><Plus size={16}/> Add vendor</button></div></div><section className="panel table-panel"><div className="toolbar"><div className="search-box"><Search size={16}/><input value={query} onChange={e=>setQuery(e.target.value)} placeholder="Search vendors"/></div></div><ResourceTable columns={columns} rows={rows} loading={loading} error={error}/></section><Modal open={open} title="Create vendor" description="Add a production partner or supplier." onClose={()=>setOpen(false)} width="720px"><VendorForm onSubmit={submit} submitting={submitting}/></Modal></>;
}
