...
const userCreateSchema = z.object({ name: z.string().min(2).max(150), email: z.string().email().max(191), password: z.string().min(8).max(200), roleId: z.coerce.number().int().positive(), status: z.enum(['ACTIVE', 'INACTIVE']).optional() });
const userUpdateSchema = z.object({ name: z.string().min(2).max(150).optional(), email: z.string().email().max(191).optional(), password: z.string().min(8).max(200).optional(), roleId: z.coerce.number().int().positive().optional(), status: z.enum(['ACTIVE', 'INACTIVE']).optional() });
...
const companySettingsSchema = z.object({ companyName: z.string().min(2).max(200), legalName: z.string().max(200).optional().nullable(), phone: z.string().max(50).optional().nullable(), email: z.string().email().max(191).optional().nullable(), address: z.string().max(5000).optional().nullable(), website: z.string().url().max(255).optional().nullable(), taxNumber: z.string().max(100).optional().nullable() });
const branchCreateSchema = z.object({ code: z.string().min(1).max(50), name: z.string().min(2).max(150), address: z.string().max(5000).optional().nullable(), phone: z.string().max(50).optional().nullable(), email: z.string().email().max(191).optional().nullable(), active: z.boolean().optional() });
...
export async function settingsRoutes(app: FastifyInstance) {
  ...
  app.put('/api/v1/settings/roles/:id/permissions', async (request, reply) => { const roleId = z.coerce.number().int().positive().parse((request.params as any).id); const input = permissionUpdateSchema.parse(request.body); const role = await prisma.role.findUnique({ where: { id: roleId } }); if (!role) return reply.notFound('Role not found'); const permissions = await prisma.permission.findMany({ where: { id: { in: input.permissionIds } }, select: { id: true } }); if (permissions.length !== new Set(input.permissionIds).size) return reply.badRequest('One or more permissions were not found'); const updated = await prisma.$transaction(async (tx) => { await tx.rolePermission.deleteMany({ where: { roleId } }); if (input.permissionIds.length) await tx.rolePermission.createMany({ data: input.permissionIds.map((permissionId) => ({ roleId, permissionId })) }); return tx.role.findUnique({ where: { id: roleId }, include: { permissions: { include: { permission: true } } } }); }); return { data: updated }; });
}
