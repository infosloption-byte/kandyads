...
const invoiceSchema = z.object({
  number: z.string().min(2).max(50), clientId: z.coerce.number().int().positive(), projectId: z.coerce.number().int().positive().optional().nullable(),
  invoiceDate: z.coerce.date(), dueDate: z.coerce.date(), subtotal: z.coerce.number().nonnegative(), discount: z.coerce.number().nonnegative().default(0),
  tax: z.coerce.number().nonnegative().default(0), total: z.coerce.number().nonnegative(), status: z.enum(['DRAFT','ISSUED','PARTIALLY_PAID','PAID','OVERDUE','CANCELLED']).default('DRAFT'), items: z.array(invoiceItemSchema).min(1),
});
const paymentSchema = z.object({ invoiceId: z.coerce.number().int().positive(), amount: z.coerce.number().positive(), paidAt: z.coerce.date(), method: z.string().min(1).max(50), reference: z.string().max(100).optional().nullable(), notes: z.string().max(1000).optional().nullable() });
...
export async function financeRoutes(app: FastifyInstance) {
  app.get('/api/v1/invoices', async (request) => {
    ...
    const data = await prisma.invoice.findMany({ where: { status: query.status as any, clientId: query.clientId, projectId: query.projectId, ...(query.q ? { OR: [{ number: { contains: query.q } }, { client: { companyName: { contains: query.q } } }] } : {}) }, include: { client: true, project: true, items: true, payments: true }, orderBy: { invoiceDate: 'desc' } });
    return { data };
  });
  ...
  app.post('/api/v1/invoices', async (request, reply) => {
    ...
    const data = await prisma.$transaction(async (tx) => {
      const invoice = await tx.invoice.create({ data: { number: input.number, clientId: input.clientId, projectId: input.projectId ?? null, invoiceDate: input.invoiceDate, dueDate: input.dueDate, subtotal: input.subtotal, discount: input.discount, tax: input.tax, total: input.total, amountPaid: 0, balance: input.total, status: input.status, items: { create: input.items } }, include: { client: true, project: true, items: true, payments: true } });
      await writeAudit(tx, { userId, action: 'INVOICE_CREATED', entity: 'Invoice', entityId: invoice.id, beforeJson: null, afterJson: { id: invoice.id, number: invoice.number, total: invoice.total.toString(), balance: invoice.balance.toString(), status: invoice.status } });
      return invoice;
    });
  ...
  app.post('/api/v1/payments', async (request, reply) => {
    ...
    const result = await prisma.$transaction(async (tx) => {
      ...
      await writeAudit(tx, { userId, action: 'PAYMENT_POSTED', entity: 'Invoice', entityId: invoice.id, beforeJson: { amountPaid: invoice.amountPaid.toString(), balance: invoice.balance.toString(), status: invoice.status }, afterJson: { paymentId: payment.id, paymentAmount: payment.amount.toString(), amountPaid: updated.amountPaid.toString(), balance: updated.balance.toString(), status: updated.status } });
