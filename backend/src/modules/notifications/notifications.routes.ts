...
import { sendNotificationDelivery } from '../../lib/notification-delivery.js';

type DeliveryRecipient = { email?: string | null; whatsapp?: string | null };

const currentUserId=(request:unknown)=>Number((request as {user?:{sub?:string}}).user?.sub);

async function insertNotification(userId:number,type:string,title:string,message:string,entityType:string,entityId:number,key:string,recipient?:DeliveryRecipient){
  const inserted=Number(await prisma.$executeRaw`INSERT IGNORE INTO Notification (userId,type,title,message,entityType,entityId,dedupeKey) VALUES (${userId},${type},${title},${message},${entityType},${entityId},${key})`);
  if(inserted && recipient) await sendNotificationDelivery({title,message,email:recipient.email,whatsapp:recipient.whatsapp});
  return inserted;
}
...
export async function notificationsRoutes(app:FastifyInstance){
  ...
  app.post('/api/v1/notifications/generate',async(request)=>{
    const userId=currentUserId(request);
    const user=await prisma.user.findUnique({where:{id:userId},select:{id:true,email:true,employee:{select:{id:true,phone:true}}}});
    if(!user)return app.httpErrors.unauthorized('Authenticated user not found');
    const recipient={email:user.email,whatsapp:user.employee?.phone};
    let created=0;
    ...
    if(user.employee){
      ...
      for(const assignment of assignments){
        created+=await insertNotification(user.id,'JOB_ASSIGNMENT','Job assigned',`You have been assigned to ${assignment.job.number} · ${assignment.job.title}.`,'JOB',assignment.jobId,`JOB_ASSIGNMENT:${user.id}:${assignment.jobId}:${assignment.id}`,recipient);
      }
      ...
      for(const task of tasks){
        ...
        created+=await insertNotification(user.id,overdue?'TASK_OVERDUE':'TASK_DUE',overdue?'Task overdue':'Task due soon',overdue?`Task ${task.title} for ${task.job.number} is overdue.`:`Task ${task.title} for ${task.job.number} is due on ${task.dueDate.toISOString().slice(0,10)}.`,'TASK',task.id,`TASK_DUE:${user.id}:${task.id}:${overdue?'OVERDUE':'DUE'}`,recipient);
      }
    }
    ...
    for(const material of materials){
      ...
      created+=await insertNotification(user.id,'LOW_STOCK','Low stock',`${material.sku} · ${material.name} is at ${stock}, at or below reorder level ${material.reorderLevel}.`,'MATERIAL',material.id,`LOW_STOCK:${user.id}:${material.id}`,recipient);
    }
    ...
    for(const invoice of invoices){
      created+=await insertNotification(user.id,'INVOICE_OVERDUE','Invoice overdue',`Invoice ${invoice.number} for ${invoice.client.companyName} is overdue with an outstanding balance of ${invoice.balance}.`,'INVOICE',invoice.id,`INVOICE_OVERDUE:${user.id}:${invoice.id}`,recipient);
    }
    ...
    if(pendingTotal>0){
      created+=await insertNotification(user.id,'APPROVAL_PENDING','Approvals pending',`${pendingTotal} approval item${pendingTotal===1?' is':'s are'} waiting for review.`,'APPROVAL',0,`APPROVAL_PENDING:${user.id}:${purchaseRequests}:${purchaseOrdersPending}:${expenses}:${outsourcing}`,recipient);
    }
