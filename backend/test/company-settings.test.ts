...
before(async () => {
  ...
  await prisma.$executeRawUnsafe(`INSERT INTO CompanySetting (id, companyName) VALUES (1, 'Kandy Ads') ON DUPLICATE KEY UPDATE id = id`);
...
test('company settings returns the singleton profile', async () => {
  ...
  assert.equal(response.body.data.companyName, 'Kandy Ads');
});
