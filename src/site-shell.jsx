...
export function Footer(){
  return <footer><div className="container footer-grid">
    <div><Link to="/" className="brand footer-brand"><img src={LOGO} alt="Kandy Ads"/><span>KANDY<span>ADS</span></span></Link><p>Creative advertising, signage and brand visibility solutions from Kandy, Sri Lanka.</p><div className="footer-contact"><a href={contactInfo.phoneHrefs[0]}><Phone size={14}/> {contactInfo.phones[0]}</a><a href={contactInfo.phoneHrefs[1]}><Phone size={14}/> {contactInfo.phones[1]}</a><a href={`mailto:${contactInfo.emails[0]}`}><Mail size={14}/> {contactInfo.emails[0]}</a><a href={`mailto:${contactInfo.emails[1]}`}><Mail size={14}/> {contactInfo.emails[1]}</a><span><MapPin size={14}/> {contactInfo.addresses[0]}</span><span><MapPin size={14}/> {contactInfo.addresses[1]}</span></div></div>
